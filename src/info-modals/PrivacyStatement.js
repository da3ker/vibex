...
const Container = styled(motion.div)`
  ${tw` rounded-lg py-8 text-justify text-gray-600 text-sm flex flex-col items-center w-4/5  overflow-y-auto bg-white outline-none border-4 border-gray-300`}
`;
const Title = styled.h1`
  ${tw`font-bold text-2xl mb-4 tracking-wider uppercase text-black`}
`;
const Wrapper = styled(motion.div)`
  ${tw`sm:px-16 px-2`}
`;
const Footer = styled.div`
  ${tw`h-12 w-full bg-gray-900 mb-[-2rem] rounded-b-lg mt-10 flex items-center justify-center`}
`;
const Logo = styled.img`
  ${tw`h-10 invert`}
`;

const PrivacyStatementModal = ({ isModalOpen, setIsModalOpen }) => {
  return (
    <Modal
      open={isModalOpen === 7}
